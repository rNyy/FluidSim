	...
	if(x++ < 100)
	  {
	    ...
	    FILE *file=fopen("Vel_u_RK.txt","a");
	    ...
	    // blueBee.record(frame_vals);
	    //  for (int i=0;i<VARS;i++)
	    // for(int i=0; i<sGrid->nY ; i++)
	    //  for(int j=0;j<sGrid->nX; j++)
		fprintf(file," %f\n ",u(5,5));
		//	     for(int i=0; i<sGrid->nY ; i++)
		//x for(int j=0;j<sGrid->nX; j++)
		fprintf(file1," %f\n ",v(5,5));
	    fclose(file);
	    fclose(file1);
	  }
...
//     q is for u and r is for v     //
/*
void FluidSim :: advect2DBoth(matrix<double>& q,matrix<double>& r, double dt, matrix<double> &u, matrix<double> &v)//keep
{
  ...
  double dx = sGrid->dx;
  //dt*=nX;
  double x,y, posx, posy,posy_v,posx_v;
  double CFL=dt/dx;
  if(CFL>1)
    CFL=0.1*CFL;
  //CFL=0.01*(dt/dx);
  double CFL2=pow(CFL,2);
  double CFL3= pow(CFL,3);
  double BWD,FWD,ERR,CURR,NEW;
  double BWD_u,FWD_u,BWD_v,FWD_v,CURR_u,CURR_v,NEW_u,NEW_v;
  double NXT_1_x,NXT_1_y,NXT_2_y,NXT_2_x,NXT_vx, NXT_vy,NXT,NXT_v;
  double PREV_x,PREV_y,PREV,PREV_vy,PREV_vx;
  double prev_x_u,prev_y_u;
  double temp1, temp2,temp1_v,temp2_v;
  double u_xy_1,u_xy_2,u_xy_11,u_xy;
#pragma omp parallel for private(x, y, posx, posy)
    for(int i=1;i<=nY-2;i++)
      {	for(int j=1;j<=nX-1;j++){
	  if((i>5 && i< nX-5)&&(j>5 && j<nY-5))
	    {
	      x = (j)*dx;
	      y = (i)*dx;
	      posx = x/dx;
	      ...
	      CURR_u = getVelInterpolated( posx, posy, u);
	      CURR_v = getVelInterpolated( posx, posy, v);

	      PREV_x = getVelInterpolated(posx-0.5,posy,u);
	      PREV_y = getVelInterpolated(posx,posy-0.5,u);
	      PREV_vx = getVelInterpolated(posx-0.5,posy,v);
	      PREV_vy = getVelInterpolated(posx,posy-0.5,v);
	      PREV = getVelInterpolated(posx-0.5, posy-0.5,u);

	      prev_x_u = getVelInterpolated(posx-1.0,posy,u);
	      prev_y_u = getVelInterpolated(posx,posy-1.0,u);

	      NXT_1_x = getVelInterpolated(posx+0.5,posy,u);
	      NXT_1_y = getVelInterpolated(posx,posy+0.5,u);

	      NXT_2_x = getVelInterpolated(posx+0.5,posy,v);
	      NXT_2_y = getVelInterpolated(posx,posy+0.5,v);

	      NXT_vx =  getVelInterpolated(posx,posy+0.5,v);
	      NXT_vy = getVelInterpolated(posx+0.5,posy,v);

	      NXT = getVelInterpolated(posx+0.5,posy+0.5,u);
	      NXT_v=getVelInterpolated(posx+0.5,posy+0.5,v);
	      
	      FWD_u = CURR_u - CURR_u*(CFL)*(CURR_u-PREV_x) - CURR_v*CFL*(CURR_u-PREV_y);
	      FWD_v = CURR_v - CURR_u*(CFL)*(CURR_v-PREV_vx) - CURR_v*CFL*(CURR_v-PREV_vy);
	      temp2 = NXT_1_y - NXT_1_y *(CFL)*(NXT_1_y-NXT) - NXT_2_y*(NXT_1_y - CURR_u );
	      temp1 = NXT_1_x - NXT_1_x *(CFL)*(NXT_1_x- CURR_u ) - NXT_2_x*(NXT_1_x- NXT);
	      BWD= FWD_u+ FWD_u*CFL*(FWD_u-temp1)+FWD_v *CFL*(FWD_u-temp2);
	      ERR =0.5*(CURR_u-BWD);
	      NEW_u = FWD_u+ERR;
	      
	      if(NEW_u >= max(CURR_u,NEW_u) || NEW_u <= min(CURR_u,NEW_u))
		NEW_u=FWD_u+0.5*ERR;
	      ...
	      FWD_u = CURR_u - CURR_u*(CFL)*(CURR_u-PREV_vx) - CURR_v*CFL*(CURR_u-PREV_vy);
	      FWD_v = CURR_v - CURR_u*(CFL)*(CURR_v-PREV_vx) - CURR_v*CFL*(CURR_v-PREV_vy);
	 
	      temp1 = NXT_2_y - NXT_1_y *(CFL)*(NXT_2_y-CURR_u) - NXT_2_y*(NXT_2_y - PREV_x );
	      temp2 = NXT_2_x - NXT_1_x *(CFL)*(NXT_2_x- NXT ) - NXT_2_x*(NXT_2_x- CURR_u);
	      BWD= FWD_v+ FWD_u*CFL*(FWD_v-temp1)+FWD_v *CFL*(FWD_v-temp2);
	      ERR =0.5*(CURR_v-BWD);
	      NEW = FWD_v+ERR;
	    
	    if(NEW >= max(CURR_v,NEW) || NEW <= min(CURR_v,NEW))
	      NEW=FWD_v+0.5*ERR;
	    r(i,j)=NEW;
	    q(i,j)=NEW_u;
	      //r(i,j)=FWD_v;  
	    ...
	    posy = y/dx;
	    RK2(posx,posy, u, v,-dt);
	    r(i,j) = getVelInterpolated(posx-0.5,posy, v);
...

{
  ...
  double CFL2=pow(CFL,2);
  double CFL3= pow(CFL,3);
  double BWD,FWD,ERR,CURR,NEW,NXT_1_x,NXT_1_y,NXT_2_y,NXT_2_x,PREV_x,PREV_y,CURR_u,CURR_v,PREV,PREV_vy,PREV_vx;
  double FWD_v,NXT,NXT_PREV_u,PREV_NXT_u,NXT_PREV_v,PREV_NXT_v;
  double prev_x_u,prev_y_u;
  double temp1, temp2;
  if(component==1)
    {
	  ...
	  if((i>3 && i< nX-3)&&(j>3 && j<nY-3))
	    {
	      ...
	      PREV_x = getVelInterpolated(posx-1,posy,u);
	      PREV_y = getVelInterpolated(posx,posy-1,u);
	      PREV_vx = getVelInterpolated(posx-1,posy,v);
	      PREV_vy = getVelInterpolated(posx,posy-1,v);
	      PREV = getVelInterpolated(posx-1, posy-1,u);
	      prev_x_u = getVelInterpolated(posx-1.0,posy,u);
	      prev_y_u = getVelInterpolated(posx,posy-1.0,u);
	     
	      NXT_1_x = getVelInterpolated(posx+1,posy,u);
	      NXT_1_y = getVelInterpolated(posx,posy+1,u);
	      NXT_2_x = getVelInterpolated(posx+1,posy,v);
	      NXT_2_y = getVelInterpolated(posx,posy+1,v);
	      NXT = getVelInterpolated(posx+1,posy+1,u);
	      
	      NXT_PREV_u = getVelInterpolated(posx+0.5,posy-0.5,u);
	      PREV_NXT_u = getVelInterpolated(posx-0.5,posy+0.5,u);
	      
	      //	  LIMITER
	      FWD = CURR_u+CURR_u*CFL*(CURR_u-PREV_x)+CURR_v*CFL*(CURR_u-PREV_y);
	      FWD_v = CURR_v+CURR_u*CFL*(CURR_v-PREV_vx)+CURR_v*CFL*(CURR_v-PREV_vy);
	      temp1= NXT_1_x+NXT_1_x*CFL*(NXT_1_x-CURR_u)+NXT_2_x*CFL*(NXT_1_x-NXT_PREV_u);//MAY TRY REPLACING NXT WITH NXT_i+1_j-1
	      temp2= NXT_1_y+NXT_1_y*CFL*(NXT_1_y-PREV_NXT_u)+NXT_2_y*CFL*(NXT_1_y-CURR_u);//TRY REPLACING NXT WITH NXT_i-1_j+1
	      
	      
	      //	      BWD = FWD+ FWD*CFL*(FWD-temp1)+FWD_v *CFL*(FWD-temp2);
	      BWD = FWD+ FWD*CFL*(FWD-temp1)+FWD_v *CFL*(FWD-temp2);
	      ERR = 0.5*(CURR_u-BWD);
	      NEW = FWD+ERR;
	     
	      
	      if(NEW >= max(CURR_u,NEW) || NEW <= min(CURR_u,NEW))
		NEW=FWD+0.5*ERR;
	      q(i,j)=NEW;
	...
	if((j>3 && j<nY-3))
	  {
	    ...
	    FWD = CURR_u+CURR_u*CFL*(CURR_u-PREV_x)+CURR_v*CFL*(CURR_u-PREV_y);
	    FWD_v = CURR_v+CURR_u*CFL*(CURR_v-PREV_vx)+CURR_v*CFL*(CURR_v-PREV_vy);
	    temp1= NXT_2_x+NXT_1_x*CFL*(NXT_2_x-CURR_u)+NXT_2_x*CFL*(NXT_2_x-NXT_PREV_u);//MAY TRY REPLACING NXT WITH NXT_i+1_j-1
	    temp2= NXT_2_y+NXT_1_y*CFL*(NXT_2_y-PREV_NXT_u)+NXT_2_y*CFL*(NXT_2_y-CURR_u);//TRY REPLACING NXT WITH NXT_i-1_j+1
	    
	    BWD = FWD_v+ FWD*CFL*(FWD-temp1)+FWD_v *CFL*(FWD-temp2);
	    ERR =0.5*(CURR-BWD);
	    NEW = FWD+ERR;
	    //	  LIMITER
	    if(NEW > max(CURR,NEW) || NEW < min(CURR,NEW))
	      NEW=FWD+0.5*ERR;
	    r(i,j)=NEW;
...
#pragma omp parallel for private(x, y, posx, posy)
	  for(int i=1;i<=nY-2;i++)
	    {
	      double bwd,error,curr, fwd,_new;
	      for(int j=1;j<=nX-1;j++){
		curr=u(i,j);
		x = (j)*dx;
		y = (i+0.5)*dx;
		...
		RK2(posx,posy, u, v,-dt);
		//	std::cout<<"\n ("<<posx<<" , "<<posy<<")\t";
		bwd = getVelInterpolated(posx,posy-0.5, u);
		error=0.25*(bwd-curr);/**/
		_new=fwd+error;
		//	std::cout<<error<<"\n";
		//CLAMP 
		
		if(_new >= max(curr,_new) || _new <= min(curr,_new))
		  _new=fwd+0.25*error;
		q(i,j)=_new;		
	      }
	    }	
...
#pragma omp parallel for private(x, y, posx, posy)
	  for(int i=1;i<=nY-1;i++)
		...
		//posy = y/dx;
		RK2(posx,posy, u, v,-dt);
		bwd = getVelInterpolated(posx-0.5,posy, v);
		//q(i,j)=bwd;
		//	q(i,j)=fwd;
		error=0.25*(bwd-curr);
			//std::cout<<bwd-curr;
...
void FluidSim :: RK2_FWD(double &posx, double &posy,matrix<double> &u, matrix<double> &v, double dt) {
  double dx = sGrid->dx;
  double x = posx*dx;
  double y = posy*dx;
  int nX = sGrid->nX;
...
void FluidSim :: initFluidBody(int FluidPos)
{
	...
	case 1: //dam_break- left right corner
	  ...
	  */
	        fluidPosBX = 1;  //last one is boundary
		fluidPosBY = 1 ;//sb+1;
		fluidPosTX = fluidPosBY + 5*(nX)/10;
		fluidPosTY = fluidPosBX + 7*(nY)/10;//nX-1;
		//fluidPosTY = ((nY-1) - (nY-1)/3) ;
